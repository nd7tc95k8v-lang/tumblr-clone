-- Optional engagement_window: count only likes/reblogs whose timestamps fall in the window.
-- Independent of time_window (post discovery). NULL engagement_window = lifetime engagement counts.

drop function if exists public.admin_top_tag_engagement(integer, interval);

create or replace function public.admin_top_tag_engagement(
  limit_count integer default 50,
  time_window interval default null,
  engagement_window interval default null
)
returns table (
  tag text,
  post_count bigint,
...
as $$
  ...
  likes_by_root as (
    ...
    from public.likes l
    where engagement_window is null
      or l.created_at >= now() - engagement_window
    group by l.post_id
  ),
  reblogs_by_root as (
    select
      p.original_post_id as root_id,
      count(*) filter (
        where p.id <> p.original_post_id
          and (
            engagement_window is null
            or p.created_at >= now() - engagement_window
          )
      )::bigint as reblog_count
    from public.posts p
...
revoke all on function public.admin_top_tag_engagement(integer, interval, interval) from public;
grant execute on function public.admin_top_tag_engagement(integer, interval, interval) to service_role;
