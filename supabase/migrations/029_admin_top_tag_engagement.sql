-- Internal admin analytics: top tags by engagement (read-only, service_role only).
-- Likes: same as feed — counts rows in public.likes where post_id is the thread root.
-- Reblogs: same as post_reblog_counts_by_root — posts with original_post_id = root, excluding the root row.

create or replace function public.admin_top_tag_engagement(limit_count integer default 50)
returns table (
  tag text,
  post_count bigint,
  total_likes bigint,
...
revoke all on function public.admin_top_tag_engagement(integer) from public;
grant execute on function public.admin_top_tag_engagement(integer) to service_role;
