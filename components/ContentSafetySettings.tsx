...
import { isAdultAccessActive, isAdultAccessExpiringSoon } from "@/lib/adult-access-expiry-window";
import {
...
import { writeContentSafetyOnboardingDismissed } from "./ContentSafetyOnboarding";
...
export default function ContentSafetySettings({ supabase, user }: Props) {
  ...
  const loadProfileContentFields = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    if (error || !data) {
      if (!silent) setProfileFieldsLoading(false);
      return;
    }
  ...
  /** Granted with a future expiry (expired or missing expiry → not active). */
  const adultAccessActive = isAdultAccessActive(adultStatus, adultExpiresAt, nowMs);
  /** Active access whose end date falls within the next 30 days. */
  const adultAccessExpiringSoon = isAdultAccessExpiringSoon(adultStatus, adultExpiresAt, nowMs);
  /** Initial attestation UI or renewal UI; hidden when access is solid for more than 30 days. */
  const showMatureContentAccessSection =
    !profileFieldsLoading && (!adultAccessActive || adultAccessExpiringSoon);

  const matureControlsUnlocked = !profileFieldsLoading && adultAccessActive;
    ...
    <div className="flex flex-col gap-5">
      {profileFieldsLoading ? (
        <p className="text-meta text-text-muted">
          Mature content is limited on this account until age verification is completed. After that, you can adjust feed
          visibility and posting defaults below.
        </p>
      ) : matureControlsUnlocked && !adultAccessExpiringSoon ? (
        <p className="text-meta text-text-muted">
          Feed preferences and posting defaults can be adjusted independently once mature content access is enabled.
        </p>
      ) : adultAccessExpiringSoon ? (
        <p className="text-meta text-text-muted">
          Your mature content access is active. Renew it below if it is close to expiring, then adjust feed visibility and
          posting defaults.
        </p>
      ) : (
        <p className="text-meta text-text-muted">
      ...
      {profileFieldsLoading ? (
        <p className="text-meta text-text-muted">Loading content settings…</p>
      ) : null}

      {showMatureContentAccessSection ? (
        <div className="flex flex-col gap-4 border-t border-border/50 pt-5">
          <div>
            <h3 className="font-heading text-base font-semibold tracking-tight text-text">Mature content access</h3>
            {adultAccessExpiringSoon ? (
              <>
                <p className="mt-1.5 text-[0.8125rem] leading-relaxed text-text-secondary">
                  Your mature content access will expire soon.
                </p>
                <p className="mt-1.5 text-[0.8125rem] leading-relaxed text-text-secondary">
                  Renew access to avoid interruption.
                </p>
                {adultExpiresAt ? (
                  <p className="mt-1.5 text-meta leading-snug text-text-muted">
                    Current access ends{" "}
              ...
              <p className="mt-1.5 text-[0.8125rem] leading-relaxed text-text-secondary">
                Mature posts are hidden or restricted by default. To view mature posts in feeds, change how mature posts
                appear in your feeds, or mark new posts as mature by default, you need to confirm you are 18 or older.
                Access is recorded for safety and must be renewed periodically.
              </p>
            )}
          </div>
          ...
          <button
            ...
            onClick={() => {
              void (async () => {
                ...
                await loadProfileContentFields({ silent: true });
              })();
            }}
            className="qrtz-btn-primary self-start px-4 py-2 text-sm"
          >
            {attestBusy ? "…" : adultAccessExpiringSoon ? "Renew access" : "Enable access"}
          </button>
        </div>
      ) : null}

      {matureControlsUnlocked ? (
        <div className="flex flex-col gap-8 border-t border-border/50 pt-6">
          ...
          <div className="flex flex-col gap-4 border-t border-border/50 pt-6">
            <div>
              <h3 className="font-heading text-base font-semibold tracking-tight text-text">Posting defaults</h3>
              <p className="mt-1.5 text-[0.8125rem] leading-relaxed text-text-secondary">
                Defaults for the composer only; they do not change existing posts.
              </p>
            </div>
            <label className="flex min-h-[44px] cursor-pointer items-start gap-2.5 text-sm text-text">
              <input
                type="checkbox"
              ...
              <span>
                ...
                <span className="mt-1 block text-meta leading-snug text-text-muted">
                  Only affects new posts you compose: the composer starts with mature marking on or off to match this.
                  You can still change mature marking on each post before you publish.
                </span>
